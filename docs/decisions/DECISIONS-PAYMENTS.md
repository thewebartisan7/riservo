...
### D-176 — `PublicBookingController::store` surfaces the race-banner 422 (`reason=online_payments_unavailable`) instead of silently downgrading to offline

- **Date**: 2026-04-24 (PAYMENTS Session 5)
- **Status**: accepted
- **Supersedes**: Session 2a's silent-offline-downgrade behaviour (not a decision ID — an implicit behaviour that was codified in the test `can_accept_online_payments false drops the online branch even when payment_mode = online`).
- **Context**: Session 2a wired the `$needsOnlinePayment` branch in `PublicBookingController::store` with a simple fall-through: if `canAcceptOnlinePayments()` returned false at submit time, the booking was created on the offline path (`confirmed + not_applicable`). The rationale was defensive — the admin might have flipped `payment_mode = online` via a dogfooding DB write while the UI was hidden (Sessions 1–4), and the fallback kept the customer's slot. Session 5's roadmap race-banner requirement ("This business is no longer accepting online payments right now — try again later or contact them directly") is incompatible with that silent downgrade: the banner is only useful if the server actually refuses the booking, and the customer silently getting a confirmed slot without paying contradicts the Business's "require online" commercial contract.
- **Decision**: the controller now computes `$customerIntendedOnline` (true when price-eligible AND (`payment_mode = online` OR (`payment_mode = customer_choice` AND `payment_choice === 'online'` — **explicit**, no null-default))), and returns **422** with `{"reason": "online_payments_unavailable", "message": ...}` BEFORE the DB transaction when `$customerIntendedOnline && !$canAcceptOnline`. The customer's slot is not held (no booking row is created) — the roadmap's "try again later" copy implies a retriable failure, not a reserved-but-downgraded slot. The soft path is preserved for all non-explicit-online signals on `customer_choice`:
    - Customer explicitly picked `payment_choice = 'offline'` → offline booking (offline was the customer's explicit choice).
    - `payment_choice` is absent (client omitted it entirely) → offline booking. In `customer_choice` mode, `booking-summary.tsx` only renders the pay-now / pay-on-site picker when `onlinePaymentAvailable === true` at page load; if the connected account was already degraded at load, the picker is not rendered and the `useHttp` payload sends no `payment_choice`. Treating that absence as "online" would escalate a degraded Stripe state into a hard 422 for customers whose UI was already showing the offline "Confirm booking" CTA. Caught by the Round-2 codex adversarial review (Finding 1).
    - Every 4xx response in `store()` + `mintCheckoutOrRollback()` is thrown via `ValidationException::withMessages([$discriminator => $localizedMessage])`. This is the Inertia-native error shape (`{errors: {field: [messages]}}`) that `useHttp` consumes into `http.errors` / `http.hasErrors`. A hand-rolled `response()->json(['reason' => ..., 'message' => ...], 422)` body is NOT populated into `http.errors` and would leave the banner dead in the browser (Round-3 codex P1 — caught only after codex ran, because the HTTP-only test suite proved the server response without proving the client rendered). The discriminator vocabulary is `slot_taken | no_provider | online_payments_unavailable | checkout_failed | provider_id | booking`, each carried as the error KEY (not a separate `reason` field). The React client renders `Object.values(http.errors)[0]` — the localized message is self-explanatory, and the key stays available for analytics if ever needed.
- **Consequences**:
    - The React `booking-summary.tsx` renders `Object.values(http.errors)[0]` — the server-localized message is enough, no client-side discriminator switch. If a future surface needs styling divergence per discriminator, `Object.keys(http.errors)[0]` carries the code.
    - The existing test `can_accept_online_payments false drops the online branch even when payment_mode = online` is rewritten to assert 422 + `Booking::count() === 0`. Two new tests prove the soft paths: `customer_choice + payment_choice = 'offline'` explicit pick still creates an offline booking, AND `customer_choice + payment_choice` absent (degraded-at-load scenario) also creates an offline booking — no 422.
